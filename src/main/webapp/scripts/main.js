// js文件隔离，closure，第一个括号写主函数，第二个表示要运行代码，匿名字执行函数
(function() {
     ...
     */
     //模拟用户名和地址
    var user_id = '0001';
    var user_fullname = 'Ada Li';
    var lng = -122.33;
    var lat = 47.60;

	// 入口函数，代码模块化，从入口开始写其他的函数
    /**
     * Initialize
     */
    function init() {
        // Register event listeners
        // 选择标签给标签添加实景并绑定，点击nearby触发的的事件
    ...
    function ajax(method, url, data, callback, errorHandler) {
	     // 实现 ajax,创建ajax object
        var xhr = new XMLHttpRequest();

		// 目的地址url就是server
        xhr.open(method, url, true);

        // 进入时检查open的状态， 判断请求request是否成功
        // 如果有data就把 data包裹起来发给server；
        xhr.onload = function() {
			// 服务器返回的状态码，判断response是否成功
        	if (xhr.status === 200) {
				// callback回调函数，意思是将函数作为参数传入ajax函数
				// 执行回调函数把response传入
